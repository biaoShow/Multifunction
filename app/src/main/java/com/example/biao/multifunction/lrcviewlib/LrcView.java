...
import static com.example.biao.multifunction.lrcviewlib.LrcViewState.Seeking;
import static com.example.biao.multifunction.lrcviewlib.LrcViewState.normal;


public class LrcView extends View implements ILrcView {
    private static final String TAG = "LrcView";
    private String NoDataMessage = "加载中...";
     ...
     * @param time 拖动到指定时间
     */
    public void seekLrcToTime(long time, boolean isResume) {
        ...
        for (int i = 0; i < mRows.size(); i++) {
            ...
            LrcRow above = i - 1 < 0 ? null : mRows.get(i - 1);

            if (isResume) {
                if (above != null && time > above.CurrentRowTime && time < current.CurrentRowTime) {
                    StartMoveAnimation(current, i);
                }
            } else if ((time >= current.CurrentRowTime && next != null && time < next.CurrentRowTime)
     ...
     */
    @Override
    public void smoothScrollToTime(long time, boolean isResume) {
        seekLrcToTime(time, isResume);
    }
