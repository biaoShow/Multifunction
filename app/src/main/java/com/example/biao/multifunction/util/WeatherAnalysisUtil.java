        ...
        if (!TextUtils.isEmpty(response)) {
            ...
            if (allProvinces.length > 0) {
                for (String string : allProvinces) {
                    ...
                    DataBaseManager.getInstance(MyApplication.getContext()).insert(province);
        ...
        if (!TextUtils.isEmpty(response)) {
            ...
            if (allCities.length > 0) {
                for (String string : allCities) {
                    ...
                    DataBaseManager.getInstance(MyApplication.getContext()).insert(city);
//                    weatherDB.saveCity(city);
     ...
     * @return 返回是否处理成功
     */
    public static boolean handelCountyRespose(String response, int cityId) {
        if (!TextUtils.isEmpty(response)) {
            ...
            if (allCounties.length > 0) {
                for (String string : allCounties) {
                    ...
                    DataBaseManager.getInstance(MyApplication.getContext()).insert(county);
...
//            if (laterStr != null) {
//                Gson gson = new Gson();
//                laterWeatherList = gson.fromJson(laterStr, new TypeToken<List<LaterWeather>>() {
//                }.getType());
